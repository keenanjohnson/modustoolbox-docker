...
int CyChannelUart::readData(uint8_t data[], int size) {
    ...
    cydfuht::core::cyDebug() << QString::fromLatin1("Waiting for %1 bytes\n").arg(size);
    ...
    while (total_bytes_read.size() < size && !timeout.hasExpired(READ_WRITE_TIMEOUT_MS)) {
        ...
        if ((readbytes.size() > 0)) {
            cydfuht::core::cyDebug() << QString::fromLatin1(" Read %1 bytes\n").arg(readbytes.size());
        }
        total_bytes_read += readbytes;
    }

    cydfuht::core::cyDebug() << QString::fromLatin1(" Read operation terminated with %1 bytes\n").arg(total_bytes_read.size());
...
int CyChannelUart::writeData(uint8_t data[], int size) {
    logOperation("Write");
    const int bytes_written = (int)m_serial_port_p->write(reinterpret_cast<char *>(data), size);
...
void CyChannelUart::print() const {
    CyChannel::print();
    cydfuht::core::cyDebug() << QString::fromLatin1("Baud Rate: %1\nData bits: %2\nParity: %3\nStop bits: %4\n\n")
