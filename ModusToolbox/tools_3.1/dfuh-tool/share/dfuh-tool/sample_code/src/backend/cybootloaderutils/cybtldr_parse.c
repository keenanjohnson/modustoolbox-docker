...
int CyBtldr_ParseCyAcd2RowData(uint32_t bufSize, char* buffer, uint32_t* address, uint8_t* rowData, uint16_t* size, uint8_t* checksum) {
...
int CyBtldr_ParseCyAcdAppStartAndSize(uint32_t* appStart, uint32_t* appSize, uint32_t* dataLines, char* buf) {
    ...
    *dataLines = 0;
    ...
    do {
        ...
        if (err == CYRET_SUCCESS) {
            if (buf[0] == ':') {
                if (!appInfoFound) {
                    err = CyBtldr_ParseCyAcd2RowData(rowLength, buf, &addr, rowData, &rowSize, &checksum);
