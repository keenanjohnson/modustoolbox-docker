...
int CyBtldr_TransferData(uint8_t* inBuf, int inSize, uint8_t* outBuf, int outSize) {
    ...
    if (CYRET_SUCCESS == err && outSize != 0) err = g_comm->ReadData(outBuf, outSize);
...
static int SendData(uint8_t* buf, uint16_t size, uint16_t* offset, uint16_t maxRemainingDataSize, uint8_t* inBuf, uint8_t* outBuf, FILE* outFile) {
    uint8_t status = CYRET_SUCCESS;
...
int CyBtldr_ProgramRow(uint32_t address, uint8_t* buf, uint16_t size) {
    ...
    uint16_t maxDataTransferSize = (g_comm->MaxTransferSize >= TRANSFER_HEADER_SIZE)
                        ? (uint16_t)(g_comm->MaxTransferSize - TRANSFER_HEADER_SIZE)
                        : 0;
    if (CYRET_SUCCESS == err)
        err = SendData(buf, size, &offset, maxDataTransferSize, inBuf, outBuf, NULL);
...
int CyBtldr_CustomCommand(CyBtldr_CustomCommandData* cmdData) {
    int err = CYRET_SUCCESS;
    uint32_t PACKET_END_BYTES_SIZE = 3;  // two checkum bytes + one End Of Packet byte
    uint8_t inBuf[MAX_COMMAND_SIZE];
    uint8_t outBuf[MAX_COMMAND_SIZE];
    uint8_t outDataBuf[MAX_COMMAND_SIZE];
    uint32_t inSize;
    uint32_t outSize;
    uint8_t status = CYRET_SUCCESS;

    // prepare packet for sending
    err = CyBtldr_CreateCustomDataCmd(cmdData->data, cmdData->dataLength, inBuf, &inSize, &outSize, cmdData->cmdId);

    if (CYRET_SUCCESS == err) {
        // write sent packet to file
        if (cmdData->outFile != NULL) {
            fprintf(cmdData->outFile, "Write packet: ");
            writePacketToFile(inBuf, inSize, cmdData->outFile);
        }
        // write packet and read first 4 bytes(to get data length and read the rest)
        if (cmdData->cmdId == CMD_SEND_DATA_NO_RSP) outSize = 0;
        err = CyBtldr_TransferData(inBuf, inSize, outBuf, outSize);
        if (CYRET_SUCCESS == err && outSize > 0) {
            uint32_t outDataSize = (outBuf[2] | outBuf[3] << 8) + PACKET_END_BYTES_SIZE;
            // read the rest of the packet
            err = CyBtldr_ReadData(outDataBuf, outDataSize);
            // print the response data to CLI
            if (CYRET_SUCCESS == err) {
                if (cmdData->outCli && outDataSize > PACKET_END_BYTES_SIZE) {
                    printf("\n");
                    for (uint32_t i = 0; i < outDataSize - PACKET_END_BYTES_SIZE; i++) printf("%02x ", outDataBuf[i]);
                    printf("\n");
                }
                memcpy(outBuf + outSize, outDataBuf, outDataSize);
                outSize += outDataSize;
                // write received packet to file
                if (cmdData->outFile != NULL) {
                    fprintf(cmdData->outFile, " Read packet: ");
                    writePacketToFile(outBuf, outSize, cmdData->outFile);
                }
                err = CyBtldr_ParseCustomCmdResult(outBuf, outDataSize - PACKET_END_BYTES_SIZE, outSize, &status);
            }
        }
    }
    return err;
