...
QString& getMtbDfuConfigErrorMessage() {
    static QString m_mtbdfuConfigErrMsg;
    return m_mtbdfuConfigErrMsg;
}

void setMtbDfuConfigErrorMessage(QString& errorMsg) { getMtbDfuConfigErrorMessage() = errorMsg; }
...
void logErrorCode(int err) {
    if (err != 0) {
        cydfuht::core::cyCritical().noquote() << QString::fromLatin1("Error code: %1\n\n").arg(QString::number(err));
    }
}
...
QString cyChannelErrToString(int err) {
    switch (err) {
        ...
        case CYRET_ERR_FILE:
            return QObject::tr("Error: cannot access the input file. Verify that it exists and has read permissions enabled.");
        case CYRET_ERR_EOF:
            return QObject::tr("Error: reached the end of the file.");
        case CYRET_ERR_LENGTH:
        case CYRET_ERR_BTLDR_MASK | CYBTLDR_STAT_ERR_LENGTH:
            return QObject::tr("Error: the length of data does not match expected value.");
        case CYRET_ERR_DATA:
        case CYRET_ERR_BTLDR_MASK | CYBTLDR_STAT_ERR_DATA:
            return QObject::tr("Error: the data does not match expected value.");
        case CYRET_ERR_CMD:
        case CYRET_ERR_BTLDR_MASK | CYBTLDR_STAT_ERR_CMD:
            return QObject::tr("Error: the command was not recognized.");
        case CYRET_ERR_DEVICE:
        case CYRET_ERR_BTLDR_MASK | CYBTLDR_STAT_ERR_DEVICE:
            return QObject::tr(
                "Error: the attached device is not compatible with the one specified.  The Silicon ID/Silicon Revision reported by the "
                "device do "
                "not match the value expected by the *.cyacd2 file.");
        case CYRET_ERR_VERSION:
        case CYRET_ERR_BTLDR_MASK | CYBTLDR_STAT_ERR_VERSION:
            return QObject::tr("Error: the version of the bootloader is incompatible with the host.");
        case CYRET_ERR_CHECKSUM:
        case CYRET_ERR_BTLDR_MASK | CYBTLDR_STAT_ERR_CHECKSUM:
            return QObject::tr("Error: the row checksum does not match the computed value.");
        case CYRET_ERR_ARRAY:
        case CYRET_ERR_BTLDR_MASK | CYBTLDR_STAT_ERR_ARRAY:
            return QObject::tr("Error: the flash array is not valid.");
        case CYRET_ERR_ROW:
        case CYRET_ERR_BTLDR_MASK | CYBTLDR_STAT_ERR_ROW:
            return QObject::tr("Error: the flash row is not valid for the selected array.");
        case CYRET_ERR_BTLDR:
            return QObject::tr("Error: the bootloader is not ready to process data.");
        case CYRET_ERR_ACTIVE:
        case CYRET_ERR_BTLDR_MASK | CYBTLDR_STAT_ERR_ACTIVE:
            return QObject::tr("Error: the application is currently marked as active and cannot be modified.");
        case CYRET_ERR_COMM_MASK | core::CyChannel::CYCHANNEL_UNKNOWN_ERROR:
        case CYRET_ERR_UNK:
            return QObject::tr("Error: unknown error.");
        case CYRET_ERR_COMM_MASK | CYRET_ABORT:
        case CYRET_ABORT:
            return QObject::tr("Operation aborted.");
        case CYRET_ERR_BTLDR_MASK | CYBTLDR_STAT_ERR_PROTECT:
            return QObject::tr("Error: the flash row is protected and can not be programmed.");
        case CYRET_ERR_BTLDR_MASK | CYBTLDR_STAT_ERR_APP:
            return QObject::tr("Error: the application is not valid and cannot be set as active.");
        case CYRET_ERR_BTLDR_MASK | CYBTLDR_STAT_ERR_VERIFY:
            return QObject::tr("Error: the verification of the flash row data failed.");
        case CYRET_ERR_BTLDR_MASK | CYBTLDR_STAT_ERR_KEY:
            return QObject::tr("Error: the provided key does not match the expected value.");
        ...
        case CYRET_ERR_COMM_MASK | core::CyChannel::CYCHANNEL_ACCESS_DENIED:
            return QObject::tr("Error: access was denied when trying to access the selected device.");
        case CYRET_ERR_COMM_MASK | core::CyChannel::CYCHANNEL_DEVICE_IN_USE:
            return QObject::tr("Error: the selected device is currently being used by another process.");
        ...
        case CYRET_ERR_COMM_MASK | core::CyChannel::CYCHANNEL_READ_OPERATION_ERROR:
            return QObject::tr("Error: read operation failed.");
        case CYRET_ERR_COMM_MASK | core::CyChannel::CYCHANNEL_WRITE_OPERATION_ERROR:
            return QObject::tr("Error: write operation failed.");
        case CYRET_ERR_COMM_MASK | core::CyChannel::CYCHANNEL_DEVICE_NOT_FOUND:
            return QObject::tr("Error: the selected device was not found. Please make sure that the device is connected.");
        case core::CyChannel::CYCHANNEL_MTBDFU_NOT_FOUND:
            return QObject::tr("Error: cannot access the input .mtbdfu file. For more information check the description above.");
        case core::CyChannel::CYCHANNEL_MTBDFU_CONFIGURATION_ERROR:
            return QObject::tr("Error: the input .mtbdfu file has incorrect configuration: ") + getMtbDfuConfigErrorMessage();
        default:
...
int countDataFileLines(const QString& dataFileName) {
    ...
    if (dataFile.open(QIODevice::ReadOnly)) {
        ...
        while (!dataFileStream.atEnd()) {
            bool isLine = dataFileStream.readLineInto(nullptr, 0);
            if (isLine) linesCount++;
        }
        dataFile.close();
    }
    return linesCount;
...
int getRepeatCount(int& repeatCount, QStringList& commandKeys, QString& repeatString) {
    ...
    bool isNum;
    int repeatNum = repeatString.toInt(&isNum, HEX_BASE);
    if (!isNum) {
        if (commandKeys.contains(DataFileField) && (QString::compare(repeatString, QString::fromLatin1("EoF"), Qt::CaseInsensitive) == 0)) {
            repeatCount = std::numeric_limits<int>::max();  // till the end of the file
        } else {
            cydfuht::core::cyCritical().noquote() << QString::fromLatin1(
                "Error: \"repeat\" can be either numeric or equal to \"EoF\"(only when \"dataFile\" field is defined).");
            err = CyChannel::CYCHANNEL_MTBDFU_CONFIGURATION_ERROR;
        }
    } else {
...
int copyCommandSetOptionsToCommand(QJsonObject& command, QJsonObject& session) {
    int err = CyChannel::CYCHANNEL_NO_ERROR;
    QStringList sessionKeys = session.keys();

    for (auto const& sessionKey : sessionKeys)
        if (sessionKey == DataBytesField || sessionKey == DataFileField || sessionKey == OutCliField || sessionKey == OutFileField ||
            sessionKey == FlashRowLengthField || sessionKey == StartOffsetField || sessionKey == RspField) {
            command.insert(sessionKey, session[sessionKey]);
        }
...
void writeLogTimeToFile(QString& outFileName) {
    FILE* outFile = fopen(outFileName.toStdString().data(), "a");
    if (outFile != NULL) {
        time_t timeInfo = time(NULL);
        struct tm* currentTime = localtime(&timeInfo);
        // NOLINTNEXTLINE(cppcoreguidelines-pro-type-vararg)
        fprintf(outFile, "Log time: %s", asctime(currentTime));
        fclose(outFile);
    }
}
