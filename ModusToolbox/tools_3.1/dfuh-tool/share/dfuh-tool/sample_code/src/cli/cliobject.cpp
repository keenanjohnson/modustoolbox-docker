...
CliObject::CliObject() : m_workerobject() {
    ...
    connect(this, &CliObject::workerSendCommand, &m_workerobject, &backend::WorkerObject::sendCommand);
...
void CliObject::sendCommand(const cydfuht::core::CyChannelSettings &channel, const QString &filename) {
    emit workerSendCommand(channel, filename);
}
