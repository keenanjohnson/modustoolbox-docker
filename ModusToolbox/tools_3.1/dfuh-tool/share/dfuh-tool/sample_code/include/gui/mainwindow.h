...
class MainWindow : public QMainWindow {
   ...
   protected:
    ...
    /// Generates .mtbdfu file with custom command data.
    /// \param action The action that should be written to .mtbdfu file.
    void generateMtbDfu(const QString &action);
   ...
   signals:
    ...
    /// Sends a signal to the worker object telling it to send a user-defined custom command to the device.
    /// \param channel The channel to communicate over.
    /// \param filename The file containing the custom command data.
    void workerSendCommand(cydfuht::core::CyChannelSettings channel, QString filename);
   ...
   private slots:
    ...
    void showAboutBox();
    void validateData();
    void displayFileError(QString errorStr);
   ...
   private:
    ...
    static const QLatin1String MtbdfuExt;
    static const QLatin1String HexExt;
    static const QLatin1String CyAcd2Ext;
