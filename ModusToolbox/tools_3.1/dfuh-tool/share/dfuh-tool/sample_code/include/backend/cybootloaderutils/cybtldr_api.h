...
} CyBtldr_CustomCommandResponseData;
...
} CyBtldr_CustomCommandHeaderData;

/**
 * This struct defines all of the items necessary to send custom command
 * defined by user.
 */
typedef struct {
    /** Command field. Specify the DFU command  number; Valid range 0x00 - 0xFF */
    uint8_t cmdId;
    /** Length of the payload data in bytes */
    uint8_t dataLength;
    /** Actual payload to be used in command */
    uint8_t* data;
    /** Pointer to a file thats stores intermediate packets generated by the tool. */
    FILE* outFile;
    /** Optionally output all payload data from the response packet(s) for the command to the CLI */
    char outCli;
    /** Response for a defined command. Valid range 0x00 - 0xFF. This is an optional field */
    CyBtldr_CustomCommandResponseData rsp;
} CyBtldr_CustomCommandData;
...
int CyBtldr_WriteData(uint8_t* inBuf, int inSize);
...
/*******************************************************************************
 * Function Name: CyBtldr_CustomCommand
 ********************************************************************************
 * Summary:
 *   Send custom command with specified data length. This function can be used
 *   both for applications using the .hex or string format and for commands with
 *   small string data or no data.
 *
 * Parameters:
 *   cmdData        - Data structure with custom command info(command ID, input data, etc.)
 *
 * Returns:
 *   CYRET_SUCCESS      - The command was sent successfully
 *   CYRET_ERR_LENGTH   - The result packet does not have enough data
 *   CYRET_ERR_DATA     - The result packet does not contain valid data
 *   CYRET_ERR_ARRAY	- The array is not valid for programming
 *   CYRET_ERR_ROW      - The array/row number is not valid for programming
 ...
 *
 *******************************************************************************/
EXTERN int CyBtldr_CustomCommand(CyBtldr_CustomCommandData* cmdData);
