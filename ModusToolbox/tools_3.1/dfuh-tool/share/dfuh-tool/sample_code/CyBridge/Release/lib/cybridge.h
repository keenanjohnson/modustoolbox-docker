...
/* This enum defines valid GPIO pin numbers for KitProg3 devices.
   KP3 devices with HWID 0x0D/0x09 support two pins:
   CY8CKIT-062S2-43012, CYW9P62S1-43438EVB-01, CYW9P62S1-43012EVB-01,
   CY8CKIT-064B0S2-4343W, CY8CKIT-064S0S2-4343W
   Only one gpio_port3_pin5 is supported on KP3 devices with HWID 0x0F (Hatchet-1)
*/
enum CyBridgeGpioPinsKp3 : uint8_t {
  gpio_port3_pin5 = 0x35,
  gpio_port3_pin6 = 0x36
};
...
/* The supported KitProg3 UART port */
enum cyBridgeKitProgUartPortNumber: uint8_t {
  KitProg3PrimaryUart = 0,
  KitProg3SecondaryUart = 1,
};

/* The supported values for KitProg3 UART flow control */
enum cyBridgeKitProgUartFlowControl: uint8_t {
  NoneMode = 0,
  HardwareMode = 1,
};
...
class CyBridge {
 public:
  ...
  /// \brief sets the UART flow control mode, used in KitProg3 and target device communication
  /// \param port_number the KitProg3 UART port
  /// \param mode the mode of the UART flow control to set to
  /// \return the status of the operation as the standard error_code
  virtual int32_t setUartFlowControl(cyBridgeKitProgUartPortNumber port_number, cyBridgeKitProgUartFlowControl mode) = 0;

  /// \brief retrieves the UART flow control mode set for the requested KitProg3 UART port
  /// \param port_number the KitProg3 UART port
  /// \param mode the current flow control mode of the KitProg3 UART
  /// \return the status of the operation as the standard error_code
  virtual int32_t getUartFlowControl(cyBridgeKitProgUartPortNumber port_number, cyBridgeKitProgUartFlowControl& mode) = 0;

    // RTT
  /// \brief initializes the objects required for the RTT interface.
  /// \return error code
   virtual int32_t initRtt() = 0;

  /// cleans the objects required for the RTT interface
  /// \return error code
   virtual int32_t cleanRtt() = 0;

  /// \brief attempts to make a connection to hostName on the given port
  /// \param hostName hostName may be an IP address in string form (e.g., "43.195.83.32"), or it may be a host name (e.g., "example.com")
  /// \param port TCP port
  /// \return the status of the operation as the standard error_code
   virtual int32_t rttConnectSocketToHost(const char* hostName, uint16_t port) = 0;

    /// \brief closes the socket
    /// \return the status of the operation as the standard error_code
   virtual int32_t rttDisconnect() = 0;

    /// \brief reads data from the RTT protocol until the provided buffer is filled. It is a blocking function that does not
    /// exit until a timeout or receipt of the expected number of bytes
    /// \param buff_p - the pointer to the Read buffer
    /// \param buff_len - the number of bytes to read, must be <= of the size of the buff_p buffer
    /// \param timeout_ms - the timeout for waiting for Read data. The default value is 1000 ms
    /// \return if >= 0 number of read bytes (=buff_len), otherwise - error code
   virtual int32_t rttRead(uint8_t* buff_p, uint32_t buff_len, uint32_t timeout_ms = DefaultTimeout) = 0;

    /// \brief sends raw data over RTT protocol
    /// \param buff_p - the pointer to data to be sent
    /// \param buff_len - the number of bytes to send
    /// \return error code
   virtual int32_t rttWrite(const uint8_t* buff_p, uint32_t buff_len) = 0;

    /// \brief check if socket is connected to host
    /// \return 0 if socket is not connected to host, 1 if socket is connected to host else < 0 (error code)
   virtual int32_t rttIsActive() const = 0;

    /// \brief switches read data mode
    /// \param readyReadEnable - when true, new data available for reading from the device's current read channel will be read and added to local buffer.
    /// This is valid until the next call to this function
    /// \return error code
   virtual int32_t rttSetReadyReadEnable(bool readyReadEnable) = 0;

    /// \brief clears all data buffers
    /// \return error code
   virtual int32_t rttClearData() = 0;
};
