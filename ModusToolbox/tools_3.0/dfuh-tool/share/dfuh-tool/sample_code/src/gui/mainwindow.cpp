...
MainWindow::MainWindow() : m_ui(), m_filterdialog(this), m_fileerror_p(nullptr), m_i2caddresserror_p(nullptr), m_workerobject() {
    ...
    for (int divisor = 2; divisor < 256; ++divisor) {
        int hertz = std::lround(12000000.0 / divisor);
        if (hertz >= 1000000) {
    ...
    qInfo() << "settingsFileName=" << QDir::fromNativeSeparators(settings.fileName());
    ...
    connect(this, &MainWindow::workerClose, &m_workerobject, &backend::WorkerObject::close);
...
void MainWindow::findCyacd2File() {
    QString filename = QFileDialog::getOpenFileName(
        this, tr("Open firmware file"), QFileInfo(m_ui.m_file_lineedit_p->text()).absolutePath(), tr("Cyacd2 files (*.cyacd2)"));
    if (filename.isEmpty()) {
        return;
    }
...
void MainWindow::workerReturnValue(int rval) {
    ...
    enableOrDisableActions();
    if ((rval != CYRET_SUCCESS) && (rval != CYRET_ABORT)) {
...
void MainWindow::enableOrDisableActions() {
    bool filevalid = QFile(m_ui.m_file_lineedit_p->text()).exists();
    ...
    m_ui.m_verify_action_p->setEnabled(filevalid && portvalid && m_startofoperation.isNull());
    m_ui.m_eraseall_action_p->setEnabled(filevalid && portvalid && m_startofoperation.isNull());
    m_ui.m_abort_action_p->setEnabled(!m_startofoperation.isNull());
}

void MainWindow::validateData() {
    if (!QFile(m_ui.m_file_lineedit_p->text()).exists()) {
        QString errorstr = tr("File does not exist.");
